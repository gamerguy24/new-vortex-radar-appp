...
**Echo Radar** is developed and maintained by **David Wallis** under **Twistcaster Live Media LLC**.
...
npm run start
```
The dev server runs on port `3333`, so open `localhost:3333` to view your local copy of Echo Radar.
...
## Credits
Echo Radar builds on open-source radar parsing work. The libraries that parse NEXRAD files client-side were ported to JavaScript from two Python packages:
...
## ⚠️ Disclaimer ⚠️
Echo Radar is a hobby/proof-of-concept project that does not follow rigorous safety testing, so the accuracy or 24/7 availability of the data is not always guaranteed.

🚨 **Echo Radar should NEVER be used for life-saving information! ALWAYS go to the NWS or other sources with accurate data and information!!!** 🚨
