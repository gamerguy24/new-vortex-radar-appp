/*
 * 3d/marching_cubes.js
 * A compact, dependency-free marching-cubes isosurface extractor over a regular
 * 3D scalar field (Lorensen-Cline lookup tables). Given a Float32 grid of
 * reflectivity (dBZ) and an isolevel, it returns interleaved triangle vertex
 * positions in grid coordinates. Runs inside the storm-3D worker.
 *
 * (c) David Wallis, Twistcaster Live Media LLC 2026
 */

// edgeTable[cubeindex] -> which of the 12 cube edges are intersected.
...
// triTable[cubeindex][i] -> up to 5 triangles (16 entries, -1 terminated),
// each listing 3 edge indices. Stored flat (256*16).
const TRI_TABLE = buildTriTable();

// The 12 edges as pairs of corner indices (0..7).
const EDGE_CORNERS = [
 ...
 * @param {Float32Array} field  length nx*ny*nz, index = x + nx*(y + ny*z)
 * @returns {{ positions: Float32Array }} positions in grid space.
 */
export function marchingCubes(field, nx, ny, nz, iso) {
  const positions = [];
  ...
  for (let z = 0; z < nz - 1; z++) {
    for (let y = 0; y < ny - 1; y++) {
      for (let x = 0; x < nx - 1; x++) {
        let cubeindex = 0;
        for (let c = 0; c < 8; c++) {
          const o = CORNER_OFFS[c];
          const v = field[idx(x + o[0], y + o[1], z + o[2])];
          corner[c] = v;
        ...
        for (let e = 0; e < 12; e++) {
          if (edges & (1 << e)) {
            ...
            const oa = CORNER_OFFS[a], ob = CORNER_OFFS[b];
            vertList[e] = [
              x + oa[0] + t * (ob[0] - oa[0]),
              y + oa[1] + t * (ob[1] - oa[1]),
              z + oa[2] + t * (ob[2] - oa[2]),
        ...
        for (let i = 0; TRI_TABLE[base + i] !== -1 && i < 16; i += 3) {
          const e0 = vertList[TRI_TABLE[base + i]];
          const e1 = vertList[TRI_TABLE[base + i + 1]];
          const e2 = vertList[TRI_TABLE[base + i + 2]];
          if (!e0 || !e1 || !e2) continue;
          positions.push(e0[0], e0[1], e0[2], e1[0], e1[1], e1[2], e2[0], e2[1], e2[2]);
        }
      }
    }
  }
...
// Canonical Lorensen-Cline triangle table, decoded to a flat 256*16 Int8 array.
...
[9,5,4,0,8,3],[0,5,4,1,5,0],[8,5,4,8,3,5,3,1,5],[1,2,10,9,5,4],[3,0,8,1,2,10,4,9,5],[5,2,10,5,4,2,4,0,2],
[2,10,5,3,2,5,3,5,4,3,4,8],[9,5,4,2,3,11],[0,11,2,0,8,11,4,9,5],[0,5,4,0,1,5,2,3,11],
[2,1,5,2,5,8,2,8,11,4,8,5],[10,3,11,10,1,3,9,5,4],[4,9,5,0,8,1,8,10,1,8,11,10],[5,4,0,5,0,11,5,11,10,11,0,3],
...
[1,6,5,1,2,6,3,0,8],[9,6,5,9,0,6,0,2,6],[5,9,8,5,8,2,5,2,6,3,2,8],[2,3,11,10,6,5],[11,0,8,11,2,0,10,6,5],
...
[0,1,9,4,7,8,2,3,11,5,10,6],[9,2,1,9,11,2,9,4,11,7,11,4,5,10,6],[8,4,7,3,11,5,3,5,1,5,11,6],
[5,1,11,5,11,6,1,0,11,7,11,4,0,4,11],[0,5,9,0,6,5,0,3,6,11,6,3,8,4,7],[6,5,9,6,9,11,4,7,9,7,11,9],
[10,4,9,6,4,10],[4,10,6,4,9,10,0,8,3],[10,0,1,10,6,0,6,4,0],[8,3,1,8,1,6,8,6,4,6,1,10],[1,4,9,1,2,4,2,6,4],
[3,0,8,1,2,9,2,4,9,2,6,4],[0,2,4,4,2,6],[8,3,2,8,2,4,4,2,6],[10,4,9,10,6,4,11,2,3],[0,8,2,2,8,11,4,9,10,4,10,6],
...
[3,0,8,11,7,6],[0,1,9,11,7,6],[8,1,9,8,3,1,11,7,6],[10,1,2,6,11,7],[1,2,10,3,0,8,6,11,7],[2,9,0,2,10,9,6,11,7],
[6,11,7,2,10,3,10,8,3,10,9,8],[7,2,3,6,2,7],[7,0,8,7,6,0,6,2,0],[2,7,6,2,3,7,0,1,9],[1,6,2,1,8,6,1,9,8,8,7,6],
[10,7,6,10,1,7,1,3,7],[10,7,6,1,7,10,1,8,7,1,0,8],[0,3,7,0,7,10,0,10,9,6,10,7],[7,6,10,7,10,8,8,10,9],
[6,8,4,11,8,6],[3,6,11,3,0,6,0,4,6],[8,6,11,8,4,6,9,0,1],[9,4,6,9,6,3,9,3,1,11,3,6],[6,8,4,6,11,8,2,10,1],
[1,2,10,3,0,11,0,6,11,0,4,6],[4,11,8,4,6,11,0,2,9,2,10,9],[10,9,3,10,3,2,9,4,3,11,3,6,4,6,3],
...
[3,1,10,11,3,10],[1,2,11,1,11,9,9,11,8],[3,0,9,3,9,11,1,2,9,2,11,9],[0,2,11,8,0,11],[3,2,11],
[2,3,8,2,8,10,10,8,9],[9,10,2,0,9,2],[2,3,8,2,8,10,0,1,8,1,10,8],[1,10,2],[1,3,8,9,1,8],[0,9,1],[0,3,8],[]];
  const flat = new Int8Array(256 * 16).fill(-1);
  for (let i = 0; i < 256; i++) {
    const row = T[i];
    for (let j = 0; j < row.length; j++) flat[i * 16 + j] = row[j];
  }
  return flat;
}
