...
// On-screen size (px) of the TVS / hail signature triangle icons. Adjust this to
// make both signature icons bigger or smaller; the triangle is drawn at this
// resolution so it stays crisp.
const SIGNATURE_ICON_SIZE = 22;
...
class StormCentersLayer {
    ...
    _createAndRegisterIcon(map, iconName, color, inverted = false) {
        try {
            const size = SIGNATURE_ICON_SIZE;
            const inset = Math.max(2, Math.round(size * 0.0625));
            ...
            ctx.lineWidth = Math.max(1.5, size * 0.0625);
            ctx.beginPath();
            ctx.moveTo(points[0][0], points[0][1]);
            ctx.lineTo(points[1][0], points[1][1]);
            ctx.lineTo(points[2][0], points[2][1]);
