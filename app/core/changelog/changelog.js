...
const CHANGELOG = [
    {
        date: 'August 28, 2026',
        items: [
            { title: 'Split screen works like a real compare view', desc: 'Click a pane to select it, then use the normal product menu — whatever you pick lands in the pane you clicked, and the other one is left alone. The selected pane is outlined so you can see which one you are driving. Opening split screen now puts the same radar in both panes straight away, and changing station moves both while each keeps its own product. The separate right-pane control box is gone; everything runs from the app&rsquo;s own controls.' },
