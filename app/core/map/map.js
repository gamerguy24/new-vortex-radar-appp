...
// Expose the map to the standalone ES-module feature layers (cams, MRMS,
// spotters, weather reports). They were written against MapLibre, whose
// Marker/Popup/source APIs are compatible with Mapbox GL, so we alias the
// global. window.vortexMap is a small wrapper matching their expected shape.
if (typeof window !== 'undefined') {
    if (!window.maplibregl && typeof mapboxgl !== 'undefined') { window.maplibregl = mapboxgl; }
    window.vortexMap = { map: map, dualMap: null };
